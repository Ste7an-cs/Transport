...
- **💥 破坏性** **`TcpClientTransport` 与 `TcpClientConfig` 删除**,并入 `TcpTransport`(**D1**)。重连是 TCP 客户端的**固定语义**(RT_TCP_RECONNECT_001 明确"不设运行时开关"),做成外层泵的一部分即可,无需单设包装类。原分层的理由是"`TcpServer` 复用裸管道",但服务端连接**不重连**、外层泵形态本就不同,复用的是数据面而非泵形态。
- **💥 破坏性** **删除** `State()` / `WaitForState()` / `Generation()` / `AttemptCount()` / `LastFailure()` 五个 TCP 独有的诊断方法(**D9**),与 ADR-0008 D10"一个事实一条出口"同向。对外只剩 `CurrentLinkState()` 的 `LinkState` 三值;代际转为**内部记账**。
- **💥 破坏性** **`TcpClientConfig` 的三个时间量合并为一个**(**D5**):`connect_timeout` / `reconnect_interval` / `silence_timeout` → 单一 `silence_timeout`,三处共用(等连上 / 读静默判链路坏 / 连不上时的重连退避)。与 `UdpTransport` 逐字相同。**须为正,不再有"0 = 禁用"这一档**——它同时是退避间隔,零值会退化为紧循环。

**形态**

- **变更** `TcpTransport` 改为「**socket 管理泵 + 内层数据泵 + 读写双队列**」,与 `UdpTransport` 同构(**D2**)。两条队列**不随连接重建而更换**,故重连对调用方完全透明。**不自终**——连接失败、读流终止、静默超时一律回外层重试,唯一退出条件是我方 `Close`。
- **变更** **整个生命期一个 `QTcpSocket`**(**D3**):每轮末尾无条件 `abort()`,socket 回 `UnconnectedState`,下轮在**同一对象**上重连。与 UDP 的 bind→close→再 bind 同模式,**不需要每代际新建对象**。
- **变更** 判活以**对端断开事件为主判据**、静默超时为辅(半开检测,**D4**)——与 UDP **相反**(后者无连接、无断开事件,静默超时是其唯一判据)。断链**不需要单独订阅**:`readAll()` 在 `disconnected` 与 socket error 上**都先 `drain()` 残留字节再关流**,故经流的自然终止到达且**尾字节不丢**。
- **变更** `Datagram::peer` 读写两侧一律**固定对端**(**D8**)。写侧**忽略**调用方填的 `peer` 且**不判 `kInvalidArgument`**——这与 UDP **相反**(UDP 解析不了目的地会丢该条并记 `LastError`),目的是让"传输无关的调用方"换传输即可运行。

**两处经实测才定下来的设计**

- **`socket_->abort()` 不是打断手段**(**D15**)。实测:socket 处于 `ConnectingState` 时 `abort()` 对 `waitForConnected()` 与 `readAll()` 流**双双唤不醒**,均挂满 3000ms;而持句柄 `close()` **1 毫秒内**两处都醒。成因是 Qt 的 `abort()` 在连接中的 socket 上**不发 `errorOccurred`**。故 `connect_waiter_` 与 `read_stream_` **持为成员**,`Close()` 逐个 `close()`;`abort()` 降为清理动作。
  **`Close()` 共五处打断,缺一不可**——负向对照实测:去掉任一处,收敛即退化为挂满一个 `silence_timeout`(读侧三处 1800–2949ms),而**写侧两处更严——直接永久挂死**(它们是无限期 `await`,而 `WaitClosed()` join 管理泵、管理泵收尾又要先 join 写泵)。
- **写泵不等刷出**(**D13**):`write()` 交给 Qt 内部缓冲即算完成,**不等 `bytesWritten`、不等 `bytesToWrite() == 0`**。由此写泵**没有任何挂起点**——`UdpTransport` 头注释里"该不变式**只对 UDP 成立**(串口/TCP 的写有挂起点)"这句**已被消除**,两个写泵结构同构(该注释已同步更正,**串口未重构、仍待核**)。
  负向对照:加回 `waitForBytesWritten` 后,4 MiB 灌给不读的对端时 `Close` 由 **11ms → 165 460ms**。
  **代价(明确接受)**:Qt 内部写缓冲**无上限**——这是有界 `write_queue_` 挡不住的那一段。设上界需 `setWriteBufferSize(N)`,但那会引入真正的短写,须连同 D13 与 D7 重新评审,本轮不设。

**队列策略:推翻 ADR-0007 D6 的预设**

- **决策** 字节流介质**沿用同一套**「有界 1024 + 静默丢最旧」,**不另设策略**(**D6** / SDD **DD-15**)。ADR-0007 D6 原写"字节流丢中段即帧错乱,TCP/串口跟进时**必须**另定策略"——该预设**不成立**,因为丢弃后果有**两层现成补救**:codec 逐字节重同步(`ScanSystemFrames` 扫帧头、CRC 不匹配右移一字节继续扫,**只毁跨越丢弃点的那一帧**)与三个 `RequestFor*` 的重发。
  且经核实:`FiberChannel` **无公开 `size()`、无阻塞 push、丢弃静默**(`push` 挤掉队首仍返 `success`)——**溢出本身不可观测**,故"溢出即断链"在当前原语上**根本无法实现**。
  **代价**:`Send`(noresponse)**不在第二层补救内**,它无重试、丢弃即永久丢失,故队列丢弃对四种交互的后果**不均等**。

**测试面**

- **变更** 八个停摆的 TCP 用例(2812 行)**逐个判定**:三个迁移回编译面(7 条断言)、三个删除(每条给出取代者或撤销依据)、两个继续停摆(`ApplyConfig` 待 **D11** 裁决、`TcpServer` 归 **D10**)。净删 2128 行、新增 679 行。
- **修复** #132(夹具竞态)随之关闭——迁移件每处 `Accept()` 后都先 `pumpFiberUntil(kUp)` 再操作传输,根因未被搬进编译面。

**未做(明确划出范围)**

- **`TcpServer` 不在本轮**(**D10**):服务端连接**不重连**、外层泵只跑一轮,是与客户端不同的泵形态,须单独设计。
- **`ApplyConfig` 热更新待定**(**D11**):SRS/SDD 两处均未按任一方向定稿。
- **`ITraceSink` 未接线**(#185):D9 删了五个诊断入口,唯一保留的那条出口**零发射点**,重连/断链在新形态下不可观测。`UdpTransport` 同为零发射,属传输层普遍状况。

### 有界队列丢弃不予处置,归因要求按实现校准(2026-08-28 裁决,#176/#152 关闭)

- **决策** 传输双队列与订阅信箱的溢出丢弃**不加归因、不改容量策略、不改 AsyncTask**。
- **💥 破坏性(规格)** SRS §3.6 的归因要求**整条重写**。原文两处不成立:**①「计数」自 ADR-0008 D10 起已不存在**(全部计数与时延 getter 当时已删,`ProtocolNode` 的归因函数即"只上报、不计数");**②「每一次」**被三类已定案的静默丢弃证伪(无订阅者业务帧 / 传输双队列溢出 / 订阅信箱溢出)。**`Σ(命名原因) == 总丢弃` 不再是本规格的要求。**
- **登记** `DropReason` 五项中,**编译面内只有两项有生产者**:`kBadFrame` 与 `kUnmatchedOrLateResponse`(均出自 `ProtocolNode`)。`kBusinessQueueOverflow` 与 `kCloseDrop` **全库零生产者**;`kDdsHandoffOverflow` 的唯一生产者 `DdsTransport` 不在编译面内。另 `kBadFrame` 在 `SystemDatagramCodec` 下**不可达**。
- **关闭** **TBD-009**(收发队列容量与超限处置)。其原约束"丢弃必归因"与"字节流不得丢中段"**均已解除**。

### AsyncTask 升级 `ffdda11` → `417790c`

- **变更** 带来 `Awaitable` 状态模式、`ChannelHub`、`FiberChannel::wait_peek`(非破坏性读)。
- **💥 破坏性(语义)** `Awaitable::close()` 由"只关自己那一路"改为**整流传播**——关闭 hub 表里全部消费者队列(上游同时删除了断言旧隔离语义的 `test_case_broadcast_mirror_close_isolated`)。**这正是本项目要的语义**:节点关闭即读侧终结,宿主随后关传输,两者一起关。
  **由此更正一处长期写错的表述**:`ProtocolNode` 的注释与 ADR-0008 **D5** 长期写着"`DoClose()` **只**关闭该订阅,源队列与其它订阅者**不受影响**",并据此推出"多个节点可共用一条传输"。准确说法是:多个节点**可以**共用并各得全量副本,但在关闭上**一荣俱荣**——**任一节点 `Close()` 即终结整条读流**,不支持独立关停。
- **登记** 新版 `FiberChannel` **仍无 `size()`、仍无阻塞 push、丢弃仍静默**,故 ADR-0011 **D6** 的依据不受影响。
