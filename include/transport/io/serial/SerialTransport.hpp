 ...
 *   断链判据**;线路噪声类(`ParityError`/`FramingError`/`BreakConditionError`)只落
 *   `LastError()`,**不触发设备重建**——重建**只由静默超时驱动**。噪声帧本就该由 codec
 *   的 CRC 与重同步处置(DD-15 的第一层补救)。
 ...
 * - **写不等刷出**(**D8**,与 TCP 同构):`QSerialPort::write()` 交给 Qt 内部写缓冲即返回,
 *   **不短写、也不同步刷出**。故写泵**写出段无挂起点**,三个写泵结构完全同构。
 * - **整个生命期一个 `QSerialPort`**(**D2**,概念同 ADR-0011 D3):每轮末尾 `close()`,
 *   下轮在同一对象上 `open()`,不新建对象。
 *
 * **`Close()` 是四处打断,不是 TCP 的五处**(**D6**):串口**没有“连接窗口”**,故
 * ADR-0011 **D15**(须持 `connect_waiter_` 句柄打断)**不适用**。`port->close()` 也能
 * 打断活跃读流(走 `aboutToClose`);`read_stream_` 仍持为成员并在 `Close()` 中
 * `close()`,它不依赖 Qt 的信号时序。
 *
 * **「建完即复查」同样适用**(ADR-0011 **D15 补正**,#200):`Close()` 只关得到**调用时
 * 已经存在**的句柄,泵之后才建出的读流没有任何人会唤醒,会挂满一个 `silence_timeout`。
 * 故读循环的判据即是那次复查:`while (lifecycle_ < kClosing)`。
 *
...
class SerialTransport final : public ITransport {
 public:
  ...
  ///        期间**与已关闭 → `kDown`。
  ///
  /// **只有两值,永不出现 `kEstablishing`**(**D10**):退避重开期间报 `kDown`——此刻
  /// **确实收发不了字节**;且退避是连接管理策略,不经本查询暴露。
  ///
