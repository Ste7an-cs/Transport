 ...
 * 或订阅侧加日志。
 *
 * 与各传输一致,本类面向**单线程 fiber 协作**模型:交互方法运行于调用方 fiber,读-分发
 * 循环运行于自持 fiber,二者同线程且仅在挂起点交错,而 session 取用与投递路径内均无挂起
 * 点,故普通成员不加锁——**交互方法须在节点所属执行域内调用**。
 *
 * 例外只有两处:`NodeBase` 的生命周期方法(自带互斥量),以及 `Dispatcher` 与其 `Ticket`
 * ——后者已是**跨线程安全**件(ADR-0016),故 `Subscribe()` 取回的凭据可交给普通工作线程
 * 持有、等待与析构。本类的其余成员不因此可跨线程访问。
 */
