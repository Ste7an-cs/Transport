...
```mermaid
flowchart TB
  App["应用层(用户代码)<br/>继承 DdsNode / ProtocolNode 重写钩子；或持 shared_ptr&lt；ITransport&gt； 用裸字节"]
  ...
  subgraph L1["Transport 层 — 纯字节管道(include/transport 根 + tcp/udp/serial/dds)"]
    direction TB
...
| | `SerialTransport` | 串口(QSerialPort) |
...
```
> (异步栈)`Decode`/`Post` 在 QtNetwork 传输的宿主 Qt 事件循环线程(DDS 在 listener 线程);`Dispatch` 与所有业务回调在单 worker 串行。`Encode`+`Send` 一律在锁外。协程栈(0.3.0)则 demux 与 `Request` 的 `await` 同在 fiber 调度=Qt 事件循环线程,无 worker/executor。
...
sequenceDiagram
  ...
  P->>Ep: RequestAwait(req=COMMAND, intermediate=RESPONSE, terminal=RESULT, auto_ack=RESPONSE, max=3)
  Ep->>Ep: NewCorrelation：session++，message=cmd；挂起(advanced=false)+排超时
  Ep->>Eq: COMMAND(session, message)
  Note over Ep: 超时 且 未推进(advanced 为否) 且 重发未达上限 → 重发同帧
...
sequenceDiagram
  ...
  Srv->>App: OnAccept(conn)　「conn 是独立 ITransport」
  App->>App: 在 conn 上构造节点(InteractionEngine+Policy)并 Open()；持 shared_ptr 保活
  Note over Conn: 此后该连接独立收发,与 server 解耦(server 不广播/不收发)
```

### 4.6 DDS demo:发布-订阅扇出 + 多路请求-应答

对应 [`examples/dds_node_demo.cpp`](../examples/dds_node_demo.cpp):3 节点经进程内 DDS 总线,先扇出发布、再两客户端打同一服务、应答经 `reply_to` 各回各家 inbox。

```mermaid
sequenceDiagram
  autonumber
  participant Svc as service(订 calc)
  participant Bus as DDS 总线(FakeProvider)
  participant C1 as client1(inbox c1_in,订 telemetry)
  participant C2 as client2(inbox c2_in,订 telemetry)
  Note over Svc,C2: ① 发布-订阅扇出(1 发多收)
  Svc->>Bus: publish "telemetry"(kNotify)
  par 扇出到所有订阅者
    Bus->>C1: 样本 → OnMessage(topic=telemetry)
  and
    Bus->>C2: 样本 → OnMessage(topic=telemetry)
  end
  Note over Svc,C2: ② 多路请求-应答 + 反馈(各回各家 inbox)
  C1->>Bus: publish "calc"(kRequest，corr1，reply_to=c1_in)
  C2->>Bus: publish "calc"(kRequest，corr2，reply_to=c2_in)
  Bus->>Svc: 样本 ×2(订 calc)→ OnRequest
  Svc->>Bus: 据 reply_to → publish "c1_in"(kFeedback corr1，再 kReply corr1=翻倍)
  Svc->>Bus: 据 reply_to → publish "c2_in"(kFeedback corr2，再 kReply corr2=翻倍)
  Bus->>C1: 样本(订 c1_in)→ on_feedback 再 on_terminal
  Bus->>C2: 样本(订 c2_in)→ on_feedback 再 on_terminal
  Note over C1,C2: corr + reply_to 配对 → client1 得自己的结果、client2 得自己的,不串台
```
