...
**文档标识：** transport-SDD-438C　**版本：** v2.0　**日期：** 2026-08-05
...
**需求基线：** `docs/需求规格说明书-协程原生.md`（SRS，标识前缀 `RT_`）　**决策依据：** `docs/adr/`（ADR-0001/0002/0003）
...
| REF-2 | `docs/adr/0001..0003` 架构决策记录（协程原生总纲 / 发送-丢弃-生命周期 / 目标架构与路线图 D1–D13） |
...
- **DD-6 发送完成语义 + 排序（满足 RT_TRANSPORT_007/008/004）：** 一次发送在帧字节全部进内核后才报成功，背压经协程等待自然传导；同 fiber 程序序、跨 fiber 串行为一致全序，单帧不与另一帧交错。原则：不 fire-and-forget，背压不消失。
- **DD-7 两个独立生命周期轴（满足 RT_LIFECYCLE_001/002、D3′）：** 节点生命周期 `Created→Running→Closing→Closed`（全介质通用）与 TCP 物理连接状态 `Disconnected/Connecting/Connected/Reconnecting`（仅 TCP 客户端，Running 内子状态）正交。连接概念不下沉纯字节管道基座。
...
- **DD-10 可插拔观测 + 完整性归因（满足 RT_TRACE_001/002、RT_DATA_BUFFER、D13）：** 每个丢弃点经唯一 `RecordDrop` 归因到七项 `DropReason` 之一 + 命名计数；可选 `ITraceSink` 结构化 Trace（push）与命名计数（pull）双面；未配 sink 时零控制流影响。"无静默丢失"结构性可断言（Σ命名 = 总丢弃）。
...
| **CSC_IO** | `io/`（`tcp/`·`udp/`·`serial/`·`dds/`） | `ITransport`、`IConnectionObservable`、`Tcp/Udp/Serial/DdsTransport`、`TcpClientTransport`、`TcpServer`、`IDdsProvider` | RT_TRANSPORT、RT_TCP_RECONNECT/RECONFIG、RT_IF_*、RT_IN_INTERFACE_002/003 |
...
- **用途**：介质无关的纯字节管道，向节点提供协程 await 式拉模型（`Read` 一次一片）与统一 I/O 观测面；连接管理（TCP 客户端自动重连、代际隔离）与服务端 accept。响应 RT_TRANSPORT_*、RT_TCP_RECONNECT/RECONFIG、RT_IF_*。
- **主要内容**：接口 `ITransport`（内部缝）、`IConnectionObservable`（连接观察面，不进 base）；实现 `TcpTransport`（已连接）、`TcpClientTransport`（连接管理）、`UdpTransport`、`SerialTransport`、`DdsTransport`、`TcpServer`；provider `IDdsProvider`（Fake/FastDDS）。
- **类图**：见图 4-2。
...
**图例说明**：各介质传输实现 `ITransport`；`TcpClientTransport` 额外实现 `IConnectionObservable` 并组合一代际一个内层 `TcpTransport`；`DdsTransport` 组合 `IDdsProvider`，listener 线程非阻塞 `Push` 进 `BoundedQueue<Sample>`、`Read` 侧 fiber 出队（跨线程唤醒）；`TcpServer` 每接受一条连接接管一个内层 `TcpTransport` 派生 node。
...
| D3 交互状态 | session 空闲集 `deque<uint8>` / 连接代际 `uint64` | node 各方法 | 一把 `std::mutex`（node 私有） |
...
#### 4.2.6 断连代际隔离时序（MS_GEN_ISOLATION）

**图 4-9（`seq-generation-isolation`）**

![断连代际隔离时序图](diagrams/seq-generation-isolation.svg)

**图例说明**：reactor fiber 观察连接状态机"曾 Connected 且现非 Connected"下降沿，做恰好一次旧代际隔离（`FailAll(kConnection, 不latch)` + `DrainBusinessQueue` 归因）；node 保持 Running（非 Close 终态）。
...
**图例说明**：`Disconnected/Connecting/Connected/Reconnecting` + 连接代际递增；退避 1s×2 上限 30s ±20%；端点热更新掐断当前尝试立即重试。node 侧 reactor 观察本状态机下降沿做代际隔离。
...
- **fiber（一个 node 内）**：`Start` 时 spawn 读-分发循环 fiber、handler 消费者 fiber（设 handler 时）、reactor fiber（传输为 `IConnectionObservable` 时）；`Close` 时临时 spawn finalizer fiber 驱动收敛，收敛后退出。均由 AsyncTask `makeTask` 创建、返回即终止。
- **传输连接代际**：`TcpClientTransport` connect-loop fiber 每次成功物理连接创建一个内层 `TcpTransport`（`Generation()`+1），断连销毁旧代际内层、退避后建新代际。
...
[transport]
   ...
   │ IConnectionObservable             (JK_OBSERVABLE 连接观察面, CSC_IO)
...
外部（面向使用方）接口 JK_NODE_API、JK_CODEC、JK_TRACE；内部缝 JK_TRANSPORT、JK_OBSERVABLE、JK_PROVIDER（框架自用，不对使用方开放为主 API）。
...
- 通信方式：协程 await 式拉模型；`Read` 一次一片；`Write` 帧进内核才成功。
- 协议特征：`Start/Read(options)/Write(SendUnit)/RequestClose/WaitClosed` + 强制 I/O 观测面 `LastSendTime/LastReceiveTime/LastError`；同实例同一时刻至多一个有效读。

#### 4.3.6 连接观察面（JK_OBSERVABLE）
- 优先级：中（仅连接管理传输实现，不进 base ITransport）。
- 接口类型：C++ 抽象类 `IConnectionObservable`（`io/IConnectionObservable.hpp`）。
- 数据元素：`ConnectionState`、`Generation`/`ConfigVersion`/`AttemptCount`。
- 通信方式：拉模型（`WaitStateChange` 多 fiber 并发等待）；deadline 只结束本次等待、不停后台重连。
- 协议特征：`State/WaitForState/WaitStateChange` + 代际/版本/失败/尝试诊断；node 检测本接口后 spawn reactor。

#### 4.3.7 DDS provider 抽象（JK_PROVIDER）
- 优先级：中。
...
- **观测**：`RecordDrop(reason, counter, sink)`（计数 pull + 可选 Trace push，持锁调用）；`RecordEvent(category, sink, ...)`（仅 push）；`DropReason` 七项；`TraceCategories` 九类常量单一权威。
...
- **reactor**：`RunReactorLoop` 订阅连接下降沿做代际隔离（FailAll kConnection 不 latch + DrainBusinessQueue 归因 kGenerationIsolationDrop）。
...
**单元设计决策**：各介质实现 `ITransport` 统一契约；连接管理（TCP 客户端）与纯管道分离（`TcpClientTransport` 组合内层 `TcpTransport`，一代际一实例）；DDS 跨线程有界交接闭合 ADR-0001 未决项。

**设计约束**：发送完成语义 + 背压（TCP/串口，DD-6）；UDP/DDS 单次一报文/样本，过大发送前失败；非重连传输底层致命 → Closing→Closed（D3′）；socket/串口在节点执行域 fiber 内创建（亲和纪律）。

**软件逻辑**：见 `src/io/*`。
| 单元 | 关键逻辑 |
|---|---|
| TcpTransport | 接管已连接 socket；复用 corosocket `readAll` 流；写路径帧进内核才成功 + 并发写按到达序串行化 |
| TcpClientTransport | connect-loop fiber 跑状态机 `Connecting/Connected/Reconnecting`；`abort()+deleteLater()` 管超时；退避 1s×2 上限 30s ±20%；`Read` 透明跨重连；`ApplyConfig` 单调版本原子应用 |
| UdpTransport | coroudpsocket 报文收发；`Write` 寻址 kDefault→config 默认 / kNet→ip:port；`writeDatagram` 同步原子发送 |
| SerialTransport | coroiodevice 字节流；断开过渡默认致命（TBD-005） |
...
**设计约束**：`Encode` 一对一；`Decode` 半包返回空、粘包返回多条；分帧错误 `kFrame`、语义错误 `kCodec`。
...
| MS_GEN_ISOLATION / MS_CONNECTION | 执行方案 | RT_TCP_RECONNECT、RT_LIFECYCLE_002 | §4.2.6、§4.2.8 |
...
| JK_TRANSPORT | 接口 | RT_IN_INTERFACE_002、RT_TRANSPORT | §4.3.5 |
| JK_OBSERVABLE | 接口 | RT_LIFECYCLE_002、RT_TCP_RECONNECT | §4.3.6 |
| JK_PROVIDER | 接口 | RT_IN_INTERFACE_003、RT_IF_DDS | §4.3.7 |
...
| RT_TRANSPORT_001..008 | DD-6 / CSC_IO / CSU_IO / JK_TRANSPORT / MS_DFD_TOPLEVEL |
...
| RT_LIFECYCLE_001..007 | DD-7 / CSU_NODERUNTIME / JK_OBSERVABLE / MS_CLOSE、MS_NODE_LIFECYCLE、MS_CONNECTION |
| RT_NODE_001..007 | DD-3 / CSC_NODE / CSU_PROTOCOLNODE、CSU_DDSNODE / MS_NODE_DATAFLOW |
| RT_TCP_RECONNECT_001..005 | CSU_IO、CSU_PROTOCOLNODE / JK_OBSERVABLE / MS_GEN_ISOLATION、MS_CONNECTION |
...
| RT_DESIGN_001..008 | §3 CSCI 级设计决策 DD-1..DD-10 / 全体单元 |
...
| 图 4-9 | 时序 | MS_GEN_ISOLATION | `seq-generation-isolation.mmd` |
