...
- **D6（重试次数与超时每次调用传参，不进配置）：** 以 `RetryPolicy{timeout, max_attempts}` 承载第一阶段参数，第二阶段以独立的 `result_timeout` 承载。
  **参数校验（2026-08-26 补记，#167 实现时确认）**：`max_attempts < 1`、`timeout <= 0` 与 `result_timeout <= 0` 一律返 `kInvalidArgument`。零时限在 `Ticket::Wait` 中等价于**永久等待**，而 SRS §3.1.4.4 明写"**节点不得接受『永不超时』的请求**"——时限是在途交互唯一的兜底终结源。
