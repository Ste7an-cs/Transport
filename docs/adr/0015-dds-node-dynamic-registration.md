...
1. **动态注册出来的写侧端点，首帧会静默丢失。** 这是 D15 消除、本 ADR 放回来的那个洞（背景已列实测）。后果**对四种用法不均等**：
   ...
   **框架不提供任何"何时安全"的判据**：`IDdsProvider::MatchedCount()` 是**参与者级聚合、不带 topic 参数**，节点上已有别的端点匹配着时它恒为正，对新注册的那条 topic 一无所知。故调用方**只能靠猜**（如注册后 sleep 一个经验值再发）。

   > 消除它需要给 `IDdsProvider` 再加一个**按 topic** 的匹配查询，并据此提供"注册后等到就绪"的能力。本轮**不做**（2026-09-08 裁决）。既然 **D4** 已经在动 `IDdsProvider`，将来要补，边际成本很小。
...
- **注册立即返回 + 独立的 `AwaitTopicReady(topic, timeout)`（原方案 C）。** 形状与本项目的 `Close()` / `WaitClosed()` 同构——发起与等收敛分开，要不要等由调用方按用法决定（`Publish` 该等，`RequestForResultDirect` 可以不等）。**否决理由：** 需要 `IDdsProvider` 的按 topic 匹配查询，本轮不做（2026-09-08 裁决）。**这是首帧丢失的正解，将来若要补应当走这条**。
