# ADR-0017：DDS 写侧队列改用 `FiberChannel`，四个介质的队列语义统一

**状态：** Accepted
**日期：** 2026-09-09
**关联：** **改变 ADR-0013 D3 的队列选型**（其"专属 OS 线程"的结论不变）；ADR-0016（`Dispatcher` 跨线程安全——**与本 ADR 无重叠**：那是入站分发的索引加锁，本 ADR 是出站队列换原语，两者各自独立）；ADR-0007 **D1**（读写双队列样板——本 ADR 使 DDS 真正回到该样板）；ADR-0011 **D6** / SDD **DD-15**（有界 1024 + 静默丢最旧——本 ADR 使 DDS 写侧复用该实现而非手写）。
