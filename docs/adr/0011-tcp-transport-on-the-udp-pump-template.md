...
- **D9（删除诊断面 `Generation()` / `AttemptCount()` / `LastFailure()`）：** 三个 TCP 独有的具体方法一并删除，观测只剩 `ITraceSink` 一条出口。
  依据：ADR-0008 **D10** 已把**全部**计数与时延 getter 删除（`NodeBase` 的 close_drop 与关闭时延、`ProtocolNode` 的八个观测接口、传输的收发时间戳），"一个事实一条出口"。这三个是同一批里的漏网者，没有理由单独留存。~~代际号本身**保留为内部记账**（供 Trace 归类与内部判重），只是不再对外暴露~~ —— **该保留已作废（2026-09-03，#228）**：本条登记的两个用途**双双在设计上被取消**。**Trace 归类**随 ADR-0014 **D1** 撤销观测面整条而消失，且其 **D3** 明确禁止"留以后再接的钩子"；**内部判重**从未落地——代码里唯一的判重是 `socket_ready_` 的**先清后发**（`discard_pending()` + `resolve()`，token 恒 0 或 1），由通道纪律达成、与代际号无关。**代际隔离现由"每轮末尾无条件 `abort()`"这一物理事实达成，不需要计数器。** `generation_` 成员已删（PR #229）。
  > **考古**：它在本 ADR 落地那一刻（`3e15c1a`）就已经死了——旧 `TcpClientTransport` 的两个真实消费者（`Generation()` getter 与 `RecordEvent(kTraceCategoryGeneration, …)`）都没被搬进新 `TcpTransport`，新实现从头到尾没有任何 trace 发射点。**不是 ADR-0014 造成的。**
  > **四个传输里它是唯一的死成员**（#228 逐个核过 UDP 12 / 串口 13 / DDS 14 个成员，全部有读取点）。，只是不再对外暴露——注意 **D12** 已撤销连接状态机，故它不再有"状态机需要"这一用途。
