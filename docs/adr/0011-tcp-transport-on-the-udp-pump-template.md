...
**实施范围：** `TcpTransport` 数据面与重连（三条路径的详细设计与伪代码见 SDD **§5.6.1**）。**`TcpServer` 不在本轮**（见 **D10**）；`ApplyConfig` 热更新**待定**（见 **D11**）。
...
- **D2（形态照 `UdpTransport` 样板）：** 外层 socket 管理泵 + 内层数据泵 + 读写双队列；两条队列**不随连接重建而更换**，故重连对调用方完全透明。
  ...
  写泵:
    for (;;) {
      ...
      短写循环: while (left>0) { n = write(p,left); ...; await(waitForBytesWritten) }
      刷出:     while (bytesToWrite() > 0) await(waitForBytesWritten)   // 阻塞点③ (D13)
    }
  ```
...
- **D13（写泵每条报文等刷到内核，使 Qt 内部写缓冲不成为第三条队列）：** 写泵在写完一条后**等到 `socket_->bytesToWrite() == 0`** 才取下一条。

  **依据**：`QAbstractSocket::write()` 把数据收进 **Qt 自己的内部写缓冲**即返回，几乎从不短写。若写泵不等刷出就取下一条，**Qt 内部缓冲会无界增长**——那等于凭空多出一条我们控制不了的队列，而本设计（连同 RT_TRANSPORT_010）的前提是"数据面只有两条队列"。等刷出使**背压落在有界的 `write_queue_` 上**，队列策略（**D6**）才真正管得住写侧的内存。

  **代价（明确接受）**：每条报文一次 `bytesWritten` 往返，**吞吐略降**。本项目的报文是控制指令级的量级，该代价可接受；若将来出现高吞吐场景，可改为"按水位等"（`bytesToWrite()` 超过阈值才等），届时**须重新评审本条**。

  **与 D7 的关系**：等刷出引入了挂起点（阻塞点③），期间可能断链——但 **D7 已决定不做代际号校验**，写了半条即半条。两条决策在此一致，不冲突。

  UDP 无此问题：`writeDatagram` 是同步非阻塞的单报文原子发送，**没有中间缓冲**。
...
5. **写吞吐略降。** D13 令写泵每条报文等一次 `bytesWritten` 往返。本项目报文是控制指令级量级，该代价可接受；高吞吐场景须改为"按水位等"并重新评审 D13。
