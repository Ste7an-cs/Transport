...
- **D12（配置校验，`Start()` 时一次性）：** 非法返 `kConfiguration`、**停在 `Created`**。
  ...
  > **⚠ 「四组全空返 `kConfiguration`」这一条已被 ADR-0015 D5 撤销（2026-09-08）。** 其依据是"什么都不收不发必是漏注册"，而注册可以在 `Start()` 之后补上之后该依据不再成立——"启动时还不知道有哪些 topic"正是动态注册要支持的主要场景。`DoStart()` 现在不再拒绝全空注册。
...
- **D13（`IDdsProvider` 接口增删）：**
  ...
  **不设 `UndeclareWriter`**：端点集合**启动即定型、运行期恒定**（**D16**），只在 `Shutdown()` 时整体拆除，没有单独撤销一个 writer 的时机。既有的 `Unsubscribe` 是历史遗留，不为对称而新增一个无使用者的方法。
  ...
  > **⚠ `UndeclareWriter` 已由 ADR-0015 D4 补上（2026-09-08）。** 动态注销需要撤销单个 writer，而写侧拆除此前**根本无法表达**。`IDdsProvider` 因此新增纯虚 `UndeclareWriter(topic)`（幂等），`DdsTransport` 补上 `UndeclareWriter` / `UndeclareReader`。**这对第三方 provider 是破坏性变更。**
...
- **D15（topic 端点的声明：全部在 `DoStart()`，【没有懒声明】）：** DDS 的每个 topic 需要建 `DataReader`（决定**什么会到达**）与 `DataWriter`（决定**能发往哪里**）。本设计的处置：
  ...
  **`Reply()` 不做懒声明**（2026-08-31 裁决）。**运行期不再有任何建端点的路径**——`DeclareWriter` / `DeclareReader` 只由 `DoStart()` 调用。
  ...
  > **⚠ 本条已被 ADR-0015 放开（2026-09-08）。** 注册的相位由 `Created` 放宽到 `Created ∪ Running`：`Running` 期注册即**当场建出**对应方向的端点，并配有四个 `Unregister*` 方法拆除。「端点集合启动即定型、运行期恒定」**不再成立**。
  >
  > 本 ADR 由此失去两条连带收益：`DataWriter` 不再累积（现改为**有注销故增长可控**）、回应路径不会突然吃一个 ~240ms 发现窗口（现改为**动态建出的写侧端点首帧会静默丢失**，ADR-0015 明确接受该代价，风险交由宿主评估处置）。**D3 的专属写线程、D6 的服务名派生、`reply_to` 降为交叉校验等其余部分全部沿用。**
  ...
  **`reply_to` 仍留在线缆上，但降为【一致性交叉校验】**：若 `request.reply_to` 非空且与查出的 `reply_topic` **不等**，返 `kInvalidArgument`。
  ...
  1. **`DataWriter` 不再累积。** 先前"每个出现过的 `reply_to` 永久留一个 writer、不做回收"那条代价**整条消失**——端点集合现在**完全由启动前的注册决定、启动即定型、运行期恒定**。
     > **⚠ 已随 ADR-0015 失效**：运行期可增删端点，集合不再恒定；因配有注销，增长**可控但非零**。
...
- **D16（topic 由【注册接口】给出，不进配置；仍一律在 `DoStart()` 建端点）：** 2026-08-31 裁决——把原先的四个 topic 配置字段换成四个注册方法。**`DdsNode` 的用法不变**：`Publish` / `Subscribe` / `RequestForResultDirect` / `Reply` 的签名与语义**一个字不动**（**D8**），换掉的只是"这些 topic 从哪来"。
  ...
  **端点集合仍然"启动即定型、运行期恒定"**——这一步只把"填结构体"换成"调四个函数"，**没有引入运行期动态端点**。由此：
  ...
  > **⚠ 已被 ADR-0015 推翻（2026-09-08）**：运行期动态端点正是该 ADR 引入的能力。另：本 D16 对 `cfg.*` 命名空间"框架不拦"的处置，也已由 ADR-0015 **D3** 改为**注册期硬拒**——`RegisterPublishers` / `RegisterSubscribers` 的 topic 不得以 `cfg.` 开头，违者返 `kInvalidArgument`。
