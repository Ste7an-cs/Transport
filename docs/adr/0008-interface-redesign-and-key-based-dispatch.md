...
- **D5（节点不管传输的生命周期）：** `ProtocolNode` 改为**按引用借用**传输，宿主负责其 `Start()` / `Close()` / `WaitClosed()`。节点读侧走 `AsyncRead()->shared()` 取自己的一路订阅，`DoClose()` 只关闭该订阅，源队列与其它订阅者不受影响。

  由此"节点关闭"与"传输关闭"彻底分离，也使**多个节点可共用一条传输**。代价是宿主要多写两行启停，且传输的寿命必须长于节点。
