...
flowchart TB
  ...
  subgraph L2["Codec 层(线缆格式) include/transport/codec/"]
    direction LR
    C["ICodec :: SystemCodec(协议帧/流式) · SystemDatagramCodec(协议帧/报文) · DdsCodec · LengthFieldCodec · DatagramCodec"]
  end
  ...
  subgraph Libs["底层库"]
    direction LR
    qt["Qt5 Core/Network/SerialPort"]; fastdds["Fast DDS 2.13+(可选)"]; asynctask["AsyncTask/boost.fiber(协程,可选)"]
  end
  App --> L3 --> L2 --> L1
  T -.-> qt; D -.-> fastdds
```
...
- 纯抽象核零第三方依赖;QtNetwork/QtSerialPort/Fast DDS 封在实现层/`src/`(0.3.0 起具体传输头引入 Qt 类型,协程引擎头 `transport/coro/` 引入 AsyncTask)。
...
classDiagram
  ...
  class SystemCodec
  class SystemDatagramCodec
  class DdsCodec
  class LengthFieldCodec
  class DatagramCodec
  ICodec <|.. SystemCodec
  ICodec <|.. SystemDatagramCodec
  ICodec <|.. DdsCodec
...
├── codec/   SystemCodec.hpp  SystemDatagramCodec.hpp  DdsCodec.hpp  LengthFieldCodec.hpp  DatagramCodec.hpp
...
```
> `SystemDatagramCodec`/`DatagramCodec`/`DdsCodec` 为 header-only(无对应 .cpp;`SystemDatagramCodec` 复用 `SystemCodec.cpp` 的共享帧核)。
...
### 5.1 `ITransport`
生命周期 + `Send(bytes)` / `Send(bytes, Endpoint)` + 回调注册 `OnBytes`/`OnConnect`/`OnDisconnect`。`OnBytes(Result<vector<uint8_t>>, from)` 由 QtNetwork 传输在**宿主 Qt 事件循环线程**串行调用(不自持 io 线程;DDS 在 provider listener 线程);报文式一次=一个完整 datagram/sample,流式一次=本次 read 切片(切帧归上层)。

### 5.2 TCP
- `TcpConnection`(实现 `ITransport`):包装已连接 `QTcpSocket`(`unique_ptr` 独占),`readyRead` 切片经 `OnBytes` 交付,client 与 accepted 共用;主动 `Close`(`abort`)不报 `OnDisconnect`,真断报一次(一次性闸)。
- `TcpClientTransport`(`QTcpSocket` + `QTimer`,不再继承 `TcpConnection`):`connectToHost` + 连接超时(`QTimer`)+ 指数退避自动重连。
- `TcpServerTransport`(**独立接受器,非 `ITransport`**):`QTcpServer` 监听,每 `newConnection` 取 `nextPendingConnection()`(`setParent(nullptr)` 解除父子、交 `TcpConnection` 独占防双删)经 `OnAccept(shared_ptr<ITransport>)` 交付;`OnError` 通知;`LocalPort()` 取回端口。**不广播**(用户在各连接上自行实现)。

### 5.3 UDP / 串口
- `UdpTransport`(实现 `ITransport`):单类按 `UdpMode` 配 socket(单播/组播 join_group+TTL/广播);`Send(bytes[,Endpoint::Net])`,`from` 含发送方 "ip:port"。
- `SerialTransport`(实现 `ITransport`):`QSerialPort` 配置(波特率/数据位/停止位/校验;非法即 `config:` 错);`readyRead` 切片交付,字节流,切帧归上层。
...
- **3 原语**:`Fire(Message, FrameTag, Endpoint)`(单向发,无追踪)/ `RequestAwait(Message, RequestSpec, Endpoint)`(发+登记挂起+排超时)/ `StartPeriodic(Message 或 std::function<Message()> 工厂, FrameTag, interval, Endpoint)→handle`(立即发一帧+自重排定时器;**工厂版每拍发送前【锁外】调 make() 取最新**;`UpdatePeriodic(handle, Message)` 推送换值;`StopPeriodic` 停)。`SendReply(request, FrameTag, payload)` 供节点 Responder 用。原语收 `Message`(调用方预填语义字段),policy 补相关号,引擎补 tag。
...
- **周期发布**:`StartPublishing(Message 或 sample_fn, interval, Endpoint::Topic)`=`StartPeriodic(…, kNotify)`;`UpdatePublishing`=`UpdatePeriodic`;`StopPublishing`=`StopPeriodic`。工厂版每拍发送前取最新样本。
...
- **5 模式 → 原语**(发送方法首参 `uint16_t cmd`=`message_id` 命令码,`Cmd(cmd,p)` 建 Message):`SendNoResponse`=`Fire(kCommand)`;`Request`=`RequestAwait{kCommand→kResponse, max=cfg.max_retries}`;`RequestWithResult`→`terminal=kResult`;`RequestNeedFeedback`→`intermediate=kResponse, terminal=kResult, auto_ack=kResponse`(中间 `on_response` 包成引擎 `on_intermediate`);`StartRepeating`=`StartPeriodic(kState)`(可传 `state_fn` 每拍拉最新 / `UpdateRepeating` 推送更新);心跳=`StartPeriodic(kHeartbeat)`(Open 起,首拍即发)。
...
### 7.6 可观测性 `ITraceSink`（引擎咽喉点 trace）

层中立可插拔 trace:`ITraceSink`(`include/transport/ITraceSink.hpp`,与 `ITransport`/`ICodec` 同级,避免后续 codec/transport 采用时反依赖 comm)。`InteractionEngine::SetTrace(shared_ptr<ITraceSink>)`(**Open 前**注入;设置期单线程,Open 后只读)+ 私有 `Trace()` 辅助;`DdsNode`/`ProtocolNode` 各透传一行。

- **`TraceEvent`**:零分配视图结构 `{TraceLevel level; string_view category,message,key,endpoint,error; int tag; long size; int attempt;}`,字段为指向调用点已有数据的 `string_view`,sink 若需留存自行拷贝。`TraceLevel{kTrace,kDebug,kInfo,kWarn,kError}`。
- **观测-only / 近零开销**:每个埋点 `if (trace_)` 守卫,有 endpoint 格式化的站点外层再套 `if (trace_)` → 无 sink 时不构造、不格式化、字节流逐字不变。trace 不改控制流/返回值/消息。
- **并发**:sink 可能被 io / worker / 调用方线程并发调用 → **实现须线程安全**;引擎所有 trace 在 `mu_` **外**发(`dispatch`/`timeout`/`retransmit` 在锁块后,`StopPeriodic` 锁内取 tag、锁外发)——不在锁内回调用户 sink,杜绝慢 sink 阻塞与重入死锁。
- **事件目录**:`open`·`close`(size=终结挂起数)·`conn`(connect/disconnect)·`send`·`request`·`reply`·`recv`(size=字节)·`decode`(成功 size=条数 / `decode-fail` Warn)·`dispatch`(match-terminal/match-intermediate/auto-ack)·`unmatched`(request/deliver/drop)·`retransmit`(attempt)·`timeout`·`periodic`(start/fire/stop)·`error`。
- **内置 sink**(header-only):`OstreamTraceSink`(格式化单行到 ostream,默认 `std::cerr`,内部 mutex,可设最低级别)、`CapturingTraceSink`(mutex 下深拷贝进 vector,供测试断言事件序列)。
- **预留**:`ITraceSink`/`TraceEvent` 层中立,保留 category `resync`/`frame-drop`/`io`/`reconnect` 供后续 codec/transport 加可选 sink 参数采用——引擎/接口零改动。
...
帧字节级逻辑抽成两个自由函数(`SystemCodec.hpp` 声明、`SystemCodec.cpp` 定义),流式与报文版**共用、不复制**:
- `EncodeSystemFrame(Message, CrcFn)→bytes`(编码,两版相同)。
- `ScanSystemFrames(data, len, CrcFn, out)→consumed`(从 `data[0..len)` 扫出整帧 push 进 out,返回消费字节数)。

| codec | 缓冲 | 配 | Decode |
|---|---|---|---|
| **`SystemCodec`**(流式) | 持久滚动缓冲 `buffer_` | TCP / 串口(字节流) | append→`ScanSystemFrames`→erase consumed,**保留**残留跨读拼帧 |
| **`SystemDatagramCodec`**(报文,header-only) | 无 | UDP(报文边界) | 每次 `ScanSystemFrames` 解本 datagram,**丢弃**残留、不跨报文 |

报文版适配 UDP:多对端各 datagram 独立解(不共享缓冲 → 不串台)、半截/坏报文只丢自己。

### 8.2 ProtocolNode 跑 UDP 1:多

- **回送来源**:`ProtocolConfig.reply_to_source=true` 时 `ProtocolPolicy.ReplyTo(req)` 按最后一个 `:` 切 `req.source`、`std::from_chars` 解端口 → `Endpoint::Net(host,port)`(畸形/禁用回退 `Default`)。服务端 `Responder` 回应、客户端 `needfeedback` 自动 ack(均经 `SendReply`→`ReplyTo`)都回到入站来源。引擎在 `OnBytes` 给每条入站 `m.source=from`(UDP="ip:port")。
- **指定目的地**:`ProtocolNode` 5 个发送方法加可选 `const Endpoint& to=Default()` → 转引擎原语;客户端一个 socket 发多设备,重发用 `Pending.to` 回同设备。
- **匹配**仅按 `(session_id,message_id)`(不校验来源);1:多 UDP 并发受 session 0–255 限(≤256)。默认 `reply_to_source=false`+`to=Default` → TCP/串口/1:1/流式路径逐字不变。
...
## 10. 并发与线程模型
- **Transport**:QtNetwork 传输**不自持 io 线程**,活在宿主 Qt 事件循环线程(socket 为 `unique_ptr` 成员,析构即断开所有 `connect`,`this`-捕获安全);DDS 样本在 provider listener 线程交付(同 topic 有序、跨 topic 并发)。
- **Comm(异步栈)**:`OnBytes` 线程(Qt 事件循环 / listener)内联 `Decode` → `executor.Post` → **单 worker 串行**业务回调/分发;背压在 `Post`(队列满阻塞投递方)。挂起表/计数器由 `mu_` 保护,`Encode`+`Send` 在锁外。
- **Comm(协程栈,0.3.0):** `coro::InteractionEngine` 在 fiber 调度=Qt 事件循环线程,`Request` 线性 `await_for(timeout)`(仅挂起当前 fiber),demux/等待 fiber/传输回调**同线程协作式无锁**;无 executor;`conn:`/`timeout:` 由 `channel->is_closed()` 区分;`Request` 须在固定于传输 I/O 线程的 fiber 内调。
...
## 12. 构建与依赖
- CMake;C++17。**需 Qt5**(Core/Network/SerialPort,系统安装,`find_package(Qt5 5.12 REQUIRED ...)`;传输链接 `Qt5::Core/Network/SerialPort`);nlohmann json 3.11.3 / googletest 1.14 仍 vendored 进 `third_party/`(json header-only INTERFACE,gtest `add_subdirectory`);**asio 已随 QtNetwork 迁移移除**。串口回环测试需 `socat`。
- **协程栈**(可选 `option(TRANSPORT_BUILD_CORO)`,默认 ON):`asynctask` 静态库(AsyncTask 编译单元 + `ASYNC_HAS_QTCORE` + AUTOMOC)+ 已编译 boost `fiber/context/thread/chrono`;经缓存变量 `ASYNCTASK_DIR`/`BOOST_LOCAL_ROOT` 指定路径。产出 `transport_coro` 库。
- Fast DDS 2.13+ 为可选外部依赖,`find_package(fastrtps/fastcdr QUIET)` 自动探测:未装跳过 `FastDdsProvider` 及真实互通测试,装后编译 `src/dds/FastDds*`。
