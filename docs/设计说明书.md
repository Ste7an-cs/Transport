...
classDiagram
  ...
  class InteractionPolicy { <<interface>> +TagOf()/SetTag() +NewCorrelation()/KeyOf()/EchoCorrelation() +ReplyTo() +RouteUnmatched() }
  class ProtocolPolicy; class DdsPolicy
  InteractionPolicy <|.. ProtocolPolicy
  InteractionPolicy <|.. DdsPolicy
  class InteractionEngine { +Open()/Close() +Fire() +RequestAwait() +StartPeriodic()/StopPeriodic() +SendReply() }
  ...
  class DdsNode { +Subscribe()/Unsubscribe() +Send()/Request() +OnMessage()/OnRequest() }
