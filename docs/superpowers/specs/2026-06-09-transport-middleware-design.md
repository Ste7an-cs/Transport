...
> **状态：** `ITransport` / `ITcpServer` / `IUdpTransport` / `IDdsTransport` / `TransportCore` / `TcpConnectionImpl` / `TcpClientImpl` / `TcpServerImpl` / `UdpImpl` / `SerialImpl` / `DdsImpl`（+`IDdsProvider`/`FastDdsProvider`/`DdsProviderRegistry`/`RawMessage`）已实现（Foundation + TCP + UDP + 串口 + DDS）；仅 `TransportFactory` 规划中。
>
