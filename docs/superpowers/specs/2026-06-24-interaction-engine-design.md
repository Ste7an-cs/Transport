...
- **`DdsNode`/`ProtocolNode` 在引擎上重新实现**,公开 API 与行为**不变**(纯内部重构,现有 DDS/Protocol 测试不改、保持绿)。
...
- `NewCorrelation(out)`:`out.session_id=s++; out.message_id=m++; out.protocol_id=protocol_id_; return pack(s,m)`(3 字节串)。
...
class InteractionEngine : public std::enable_shared_from_this<InteractionEngine> {
 public:
  ...
  Status   Fire(std::vector<uint8_t> payload, FrameTag tag, const Endpoint& to = Endpoint::Default());
  Status   RequestAwait(std::vector<uint8_t> payload, RequestSpec spec, const Endpoint& to = Endpoint::Default());
  uint32_t StartPeriodic(std::vector<uint8_t> payload, FrameTag tag, uint32_t interval_ms, const Endpoint& to = Endpoint::Default());
...
**三原语:**
- `Fire(payload, tag, to)`:`Message m; m.payload=payload; policy_->SetTag(m, tag); Encode; transport_->Send(bytes, to)`。无追踪。
- `RequestAwait(payload, spec, to)`:`Message out; out.payload=payload; policy_->SetTag(out, spec.request_tag);` 在 `mu_` 下 `Key k=policy_->NewCorrelation(out); pending_[k]=Pending{spec, out(留作重发), timer=ScheduleAt(超时,FireTimeout(k)), advanced=false};` 锁外 `Encode`+`Send(out_bytes, to)`(失败回滚+`on_terminal(Fail)`)。
- `StartPeriodic(payload, tag, interval, to)`:登记 handle,立即 `Fire` 一帧并排自重排定时器(每次 `Fire` 一帧 tag);`StopPeriodic` 取消。(repeating 与心跳共用)
...
**`ProtocolNode`**(持 `engine_` + `ProtocolConfig`;codec 默认 `SystemCodec`,policy=`ProtocolPolicy(protocol_id)`):
- `SendNoResponse(p)` = `Fire(p, COMMAND)`;
- `Request(p,cb)` = `RequestAwait(p,{request=COMMAND, terminal=RESPONSE, on_terminal=cb, timeout, max=cfg.max_retries})`;
- `RequestWithResult` → `terminal=RESULT`;`RequestNeedFeedback` → `intermediate=RESPONSE, terminal=RESULT, auto_ack=RESPONSE`;
- `StartRepeating(p,iv)`=`StartPeriodic(p, STATE, iv)`;`StopRepeating`=`StopPeriodic`;
...
- `Send(msg, Endpoint::Topic(t))` = `Fire(msg.payload, kNotify, Topic(t))`(发布);
- `Request(msg, cb, timeout, Endpoint::Topic(req_t))` = `RequestAwait(msg.payload, {request=kRequest, terminal=kReply, on_terminal=cb, timeout, max=0}, Topic(req_t))`;反馈重载 +`intermediate=kFeedback`;
...
- **`ProtocolNode`**:现有 `protocol_node_test.cpp` **不改**,全绿(纯内部重构验证)。
- **`DdsNode`**:现有 `dds_node_test.cpp` **不改**,全绿。
- **回归**:全量两次连跑、干净构建零告警;移除 `comm_node_test` 后总数相应变化,以 0 failed + 稳定为准。

**完成标准:** `InteractionEngine`+两 policy 落地;`ProtocolNode`/`DdsNode` 改为薄壳且**其现有测试一字不改保持绿**;`CommNode` 移除;引擎里无 per-node 特判(失败信号);并发纪律仅一处。
