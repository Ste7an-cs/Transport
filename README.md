...
- **P2 —— 节点加厚(`v0.4.2`):** 协议无关 `BoundedQueue<T>`(双上界/tail-drop/命名归因)、入站业务处理器(组合注册/单消费者 fiber 串行/异常隔离 —— RT_HANDLER 全)、`noresponse` `Send`、256 并发在途 + `session_id` LRU 退休、生命周期硬化(并发幂等 Start/多等待者/三方汇合/重入守卫 —— RT_LIFECYCLE 全)。
