...
- **P4 —— 其余介质(`v0.4.4`):** `UdpTransport`(报文+地址)/`SerialTransport`(串口字节流)/`DdsTransport`(provider 跨线程有界交接,复用 `BoundedQueue`)、`DdsNode`(pub-sub + 多路请求-应答,correlation_id 键)、`TcpServer` accept(每连接一 node)、`NodeBase`(ProtocolNode/DdsNode 共享的生命周期基类)+ `HandlerLoop`(handler 消费者与有界业务队列)。统一寻址靠 `Endpoint`。**D10 复用证实**(DdsNode 复用 PendingTable 仅一行改动)、**跨线程交接闭合 ADR-0001 未决项**。
